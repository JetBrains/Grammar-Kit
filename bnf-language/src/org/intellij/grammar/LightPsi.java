...
public class LightPsi {
  ...
  public static @Nullable PsiFile parseFile(@NotNull File file, @NotNull ParserDefinition parserDefinition) throws IOException {
    ...
    // bytesToString honours a BOM when the file has one, and strips it; UTF-8 otherwise
    String text = CharsetToolkit.bytesToString(FileUtil.loadFileBytes(file), StandardCharsets.UTF_8);
  ...
  private static class MyParsing implements Disposable {
    ...
    protected @Nullable PsiFile createFile(@NotNull String name, @NotNull String text, @NotNull ParserDefinition definition) {
      ...
      // The generator encodes its output with the grammar file's charset. A LightVirtualFile would
      // otherwise report the JVM default, making generated bytes depend on the host and on
      // -Dfile.encoding; standalone runs always read and write UTF-8.
