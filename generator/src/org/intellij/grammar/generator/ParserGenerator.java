 ...
 * {@link #myMetaMethodFields}), and the parser-only helper methods built on top of them. PSI
 * emission stays in {@link JavaPsiGenerator}; both layers share the {@code BnfFile}, rule
 * metadata, simple-token map, choice-token-set map and {@link Generator#myTokensUsedInGrammar}
 * kept on {@link Generator}.
 */
public sealed abstract class ParserGenerator extends Generator permits JavaParserGenerator, KotlinParserGenerator {
  ...
  /**
   * Resolves the consume type for emitting a call for {@code node} inside {@code rule}. A
   * non-leading sequence child whose preceding alternatives can also start at the same token is
   * forced back to {@link ConsumeType#DEFAULT} so it doesn't swallow tokens belonging to a
   * sibling alternative.
   */
