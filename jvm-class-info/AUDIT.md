...
### 14. Kotlin `suspend` and the `Continuation` parameter — **DEFERRED** (2026-05-23)
Source: `suspend fun foo(): Int`. Bytecode: `Object foo(Continuation<Integer>)`. The ASM-side reversal would need to (a) detect that the last parameter is `kotlin.coroutines.Continuation<X>`, (b) drop it, (c) restore the return type from the Continuation's type argument. Without a Kotlin compile-in-test harness (the convergence harness is Java-only today), this can't be verified end-to-end; the detection heuristic also has known false-positive risk (legitimate non-suspend methods that happen to take a Continuation, however rare). Deferred to when the Kotlin convergence harness lands.
...
### 16. `internal` → `public` mapping doesn't mangle names — **FIXED** (2026-05-23)
`AsmClassSymbolProvider.unmangleInternalKotlinName` strips Kotlin's `$<module-name>` suffix from public method names before they're stored in the ClassSymbol. Conservative heuristic: only strips when the method is `ACC_PUBLIC`, the name doesn't start with `<` (so `<init>`/`<clinit>` are untouched), and the suffix is a valid Kotlin module identifier (`[A-Za-z_][A-Za-z0-9_-]*`). Covered by `AsmInternalNameUnmanglingTest` (11 unit cases: positive matches for typical module shapes, negatives for constructors / non-public / trailing `$` / invalid suffix characters / leading `$` / digit-led suffix; multi-dollar names strip only the trailing module suffix). End-to-end Kotlin convergence pending the deferred kotlin-compiler-embeddable harness.
