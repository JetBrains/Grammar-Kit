...
### 2. Java dotted nested ref not canonicalized after head resolution
`JavaSyntaxTypeFormatter.java:343` — for `Sub.Inner.Leaf` where `Inner` is inherited from `Sub`'s supertype, the result is `enclosingFqn.Sub.Inner.Leaf` instead of the JLS-canonical `Parent.Inner.Leaf`. The canonical FQN of a nested class is its *declaring* class, not the access path.
