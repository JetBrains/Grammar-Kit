...
public class ExprParser implements PsiParser {

  public static Logger LOG_ = Logger.getInstance("ExprParser");
  ...
  private static final TokenSet[] EXTENDS_SETS_ = new TokenSet[] {
    TokenSet.create(ASSIGN_EXPR, CUSTOM_EXPR, DIV_EXPR, ELVIS_EXPR,
      EXPR, EXP_EXPR, FACTORIAL_EXPR, MINUS_EXPR,
      MUL_EXPR, OBJECT_EXPR, PAREN_EXPR, PLUS_EXPR,
  ...
  // 5: N_ARY('^')
  // 6: UNARY_POSTFIX('!')
  // 7: ATOM(identifier) UNARY('(' tail: ')') UNARY('OBJECT' '(' tail: ')')
  public static boolean expr(PsiBuilder builder_, int level_, int precedence_) {
    Marker marker_ = builder_.mark();
    boolean result_ = false;
    ...
    result_ = pinned_ && expr_0(builder_, level_, precedence_) && result_;
    if (!result_ && !pinned_) {
      marker_.rollbackTo();
    }
  ...
  public static boolean expr_0(PsiBuilder builder_, int level_, int precedence_) {
