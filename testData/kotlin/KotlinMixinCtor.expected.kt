...
object FooParser {
  ...
  /* ********************************************************** */
  // 'aa'
  fun myElement(runtime_: SyntaxGeneratedParserRuntime, level_: Int): Boolean {
    if (!runtime_.recursion_guard_(level_, "myElement")) return false
    var result_: Boolean
    val marker_: Marker = runtime_.enter_section_(level_, Modifiers._NONE_, FooSyntaxTypes.MY_ELEMENT, "<my element>")
  ...
  /* ********************************************************** */
  // 'bb'
  fun otherElement(runtime_: SyntaxGeneratedParserRuntime, level_: Int): Boolean {
    if (!runtime_.recursion_guard_(level_, "otherElement")) return false
    var result_: Boolean
    val marker_: Marker = runtime_.enter_section_(level_, Modifiers._NONE_, FooSyntaxTypes.OTHER_ELEMENT, "<other element>")
  ...
  internal fun root(runtime_: SyntaxGeneratedParserRuntime, level_: Int): Boolean {
    ...
    var result_: Boolean
    result_ = myElement(runtime_, level_ + 1)
    if (!result_) result_ = otherElement(runtime_, level_ + 1)
    return result_
  }
